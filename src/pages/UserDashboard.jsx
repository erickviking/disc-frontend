      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {tools.map(tool => {
            ...
            return (
              <div key={tool.id}
                className={`group relative flex flex-col rounded-2xl overflow-hidden transition-all duration-300 border ${isLocked ? 'border-outline-variant/10 cursor-pointer' : 'border-outline-variant/20 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5 cursor-pointer'}`}
                onClick={() => isLocked ? window.open('https://pay.hotmart.com/N105573480U?email=' + encodeURIComponent(user?.email || ''), '_blank') : navigate('/dashboard/ferramenta/' + tool.slug)}
              >
                  ...
                  {isLocked && (
                    <div className="absolute inset-0 flex items-center justify-center z-10">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-on-surface-variant bg-surface-container/80 backdrop-blur-sm px-3 py-1 rounded-full">Desbloquear</span>
                      </div>
                    </div>
                  )}
