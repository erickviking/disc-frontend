...
export default function InteligenciaEmocionalQuizPage() {
  ...
  const saveResponses = async (nextResponses) => {
    ...
    setSaving(true);
    try {
      await api.patch('/assessments/' + assessmentId + '/responses', { responses: nextResponses });
    } catch (e) {
      setError('Nao foi possivel salvar esta resposta. Verifique sua conexao antes de continuar.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-surface">
      <div className="mx-auto max-w-3xl px-4 py-8">
        <button onClick={() => navigate('/dashboard')} className="mb-6 flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-on-surface-variant hover:text-primary transition-colors">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-xs text-on-surface-variant/70">Seu progresso e salvo a cada resposta. Voce pode sair e voltar depois.</p>
        </div>
        ...
        {current && (
          <div className="card mb-6">
            <div className="flex items-center justify-between mb-6">
              ...
              {responses[current.id] !== undefined && <span className="flex items-center gap-1 text-xs font-medium text-emerald-400"><Check size={14} />Respondida</span>}
            </div>
